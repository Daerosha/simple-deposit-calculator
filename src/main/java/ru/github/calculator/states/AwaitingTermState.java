...
public class AwaitingTermState implements State {
    ...
    public AwaitingTermState(UserIO userIO, StateControl statesMachine) {
        this.userIO = userIO;
    ...
    @Override
    public void handle() {
        userIO.postValueToUser("%nEnter integer terms of months or years (no less than " + MIN_TERM + " month and no more than " + MAX_TERM / MONTH_IN_YEAR + " years)%nWhere 1m means one month and 10y means ten years, only 1 value can be used):");
        try {
            ...
            if (term >= MIN_TERM && term <= MAX_TERM) {
                statesMachine.setTerm(term);
                statesMachine.setNextState(AvaibleStates.AwaitingPercentState);
            }
